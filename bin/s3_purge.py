...
def get_archive_paths(prefix):
  level1 = flatten(list(map(lambda x: get_dir_entries(x), get_dir_entries(prefix))))
  return flatten(list(map(lambda x: get_dir_entries(x), level1)))
